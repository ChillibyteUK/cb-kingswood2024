...
<head>
	...
	<script async
		src="https://www.googletagmanager.com/gtag/js?id=<?= esc_attr( get_field( 'ga_property', 'option' ) ); ?>">
	</script>
	<script>
	...
	<script>
		(function(w, d, s, l, i) {
			...
			'<?= esc_attr( get_field( 'gtm_property', 'option' ) ); ?>'
		);
	</script>
	...
	<script type="application/ld+json">
		{
			...
			"aggregateRating": {
				...
				"bestRating": "<?= esc_attr( get_field( 'best_rating', 'options' ) ); ?>",
				"ratingValue": "<?= esc_attr( get_field( 'rating_value', 'options' ) ); ?>",
				"reviewCount": "<?= esc_attr( get_field( 'review_count', 'options' ) ); ?>"
			},
			"sameAs": [
...
<body <?php body_class(); ?>
	...
	if ( is_array( $sale_banner ) && isset( $sale_banner[0] ) && 'Yes' === $sale_banner[0] ) {
		?>
		...
		<div id="preNav">
			<div class="container-xl">
				...
				<div class="button-container text-end d-flex d-lg-none align-items-center justify-content-between">
					<a href="tel:<?= esc_attr( parse_phone( get_field( 'contact_phone', 'option' ) ) ); ?>"
